  ...
  public:
    ...
    constexpr LongInt &operator-=(const LongInt &other) {
        ...
        for (int i = 0; i < get_digit_count(); ++i) {
            ...
            borrow = other.get_digits()[i] > get_digits()[i] ||
                     (other.get_digits()[i] == get_digits()[i] && borrow);
            get_digits()[i] = diff;
        }
        return *this;
    }
    ...
    constexpr LongInt &operator<<=(int shift) {
        ...
        for (int i = get_digit_count() - 1; i >= shift_digits; i--) {
            ...
            if (i - shift_digits - 1 >= 0 && i - shift_digits - 1 < get_digit_count() &&
                shift_bits != 0) {
    ...
    constexpr LongInt &operator>>=(int shift) {
        ...
        for (int i = 0; i < get_digit_count() - shift_digits; i++) {
            ...
            if (get_signed() && i + shift_digits == get_digit_count() - 1) {
                get_digits()[i] =
                    (Digit)((typename std::make_signed<Digit>::type)(low) >> shift_bits);
            } else {
                get_digits()[i] = low >> shift_bits;
            }
    ...
    constexpr LongInt& operator+=(const int64_t& other) {
        return *this += LongInt::as(*this) = other;
    }
    ...
    constexpr LongInt& operator*=(const int64_t& other) {
        LongInt b = other < 0 ? -other : other;
        LongInt result = LongInt::as(*this);
        LongInt tmp = LongInt::as(*this);

        typedef typename LargerType<Digit>::type LargeType;
        for (int i = 0; i < get_digit_count(); ++i) {
            tmp = (LargeType)get_digits()[i] * b;
            tmp <<= i * get_digit_bitlength();
            result += tmp;
        }
        if (get_signed() && is_negative() != (other < 0)) {
            result = -result;
        }
        *this = result;
        return *this;
    ...
    constexpr LongInt operator+(const int64_t& other) const {
        return *this * (LongInt::as(*this) = other);
    }
    ...
    constexpr LongInt& div_mod(const LongInt& other, bool mod) {
        if (*this < other) {
            if (!mod) {
                operator=(0);
            }
        ...
        LongInt result = LongInt::as(*this);
        result = 0;
        LongInt left_part = LongInt::as(*this);
        left_part = 0;
        for (size_t i = get_digit_count(); i > 0; --i) {
            left_part <<= sizeof(Digit) * CHAR_BIT;
            result <<= sizeof(Digit) * CHAR_BIT;
            left_part += get_digits()[i - 1];
            if (left_part < other)
                continue;
            uint64_t left = 0;
            ...
            while (left < right) {
                uint64_t middle = (uint128_t(left) + uint128_t(right)) / 2;
                if (left_part <= (other * middle)) {
                    right = middle;
                } else {
                    left = ++middle;
                }
            }
            LongInt temporary = other * left;
            if (left_part < temporary) {
                --left;
                temporary -= other;
            }
            result += left;
    ...
    constexpr LongInt& operator/=(const LongInt& other) {
        return div_mod(other, false);
    }

    constexpr LongInt& operator%=(const LongInt& other) {
        return div_mod(other, true);
    }
    ...
    constexpr explicit operator unsigned long long() const {
        ...
        memcpy(
            (void*)&result, (void*)get_digits(),
            std::min(sizeof(result), (size_t)get_digit_count() * get_digit_bitlength() / CHAR_BIT));
        if (is_negative()) {
            for (int i = get_digit_count() * get_digit_bitlength() / CHAR_BIT;
                 i < sizeof(unsigned long long); i++) {
    ...
    static std::string repeat(int count, char symbol) {
        std::string string = "";
    ...
    std::string to_string() const {
        ...
        tmp = 100000000;
        while (!copy.is_zero()) {
            std::string part = std::to_string((copy % tmp).to_int());
            if (copy >= tmp) {
                str = repeat(8 - part.size(), '0') + part + str;
            } else {
                str = part + str;
            }
            copy /= tmp;
...
template <bool _signed = true, typename Digit = unsigned char>
std::ostream& operator<<(std::ostream& output, const DynamicLongInt<_signed, Digit>& number) {
    if (number.is_negative()) {
        ...
        auto zero = DynamicLongInt<_signed, Digit>::as(number);
        output << zero - number;
        return output;
    }
    for (ssize_t i = number.get_digit_count() - 1; i >= 0; --i) {
        output << std::bitset<sizeof(Digit) * CHAR_WIDTH>{number.get_digits()[i]} << "-";
    }
    return output;
}
