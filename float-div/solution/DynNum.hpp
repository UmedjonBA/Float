...
template <typename Digit = Digit<>> class DynNum {
  public:
    ...
    DynNum(DynamicLongInt<is_signed, T> number) : DynNum(number.to_string().c_str()) {}

    DynNum(int number) : DynNum(std::to_string(number).c_str()) {}

    DynNum& operator>>=(int other) {
        if (other < 0) {
            return operator<<=(-other);
        }
        DynNum two("0.5");
        operator*=(two.binpow(other));
        return *this;
    }
